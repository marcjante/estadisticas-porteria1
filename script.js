...
function initVoiceRecognition() {
    ...
    if (location.protocol === 'file:') {
        addEventMessage(`<span style="color: #f59e0b"><i class="fas fa-exclamation-triangle"></i> Estás abriendo el archivo en local (file://). Los comandos de voz solo funcionan si la página está en GitHub Pages (https://) o en un servidor. No funcionará con doble clic al archivo.</span>`);
    }
    ...
    recognition.onstart = () => {
        addEventMessage(`<span style="color: #059669"><i class="fas fa-microphone"></i> Micrófono activo, habla ahora</span>`);
    };
    ...
    recognition.onerror = (event) => {
        ...
        let msg = `Error de voz: ${event.error}`;
        if (event.error === 'not-allowed' || event.error === 'permission-denied') {
            msg = 'Permiso de micrófono denegado. Pulsa el icono de candado/micrófono en la barra de direcciones y permite el acceso al micrófono para esta página, luego recarga.';
            isListening = false;
            const btn = $("voice-btn");
            if (btn) {
            ...
            msg = 'No se detecta ningún micrófono en este dispositivo.';
        } else if (event.error === 'network') {
            msg = 'Error de red. El reconocimiento de voz necesita conexión a internet.';
        }
        addEventMessage(`<span style="color: #dc2626"><i class="fas fa-exclamation-circle"></i> ${msg}</span>`);
    };
...
function toggleVoiceRecognition() {
    ...
    if (isListening) {
        ...
        try {
            ...
            isListening = false;
            addEventMessage(`<span style="color: #dc2626"><i class="fas fa-exclamation-circle"></i> No se pudo iniciar el micrófono: ${e.message}</span>`);
        }
    }
}
